...
export function RemoteFirewall({ nodeID, online, readAvailable, writeAvailable }: { nodeID: string; online: boolean; readAvailable: boolean; writeAvailable: boolean }) {
  ...
  return <div className="space-y-4">
    ...
    <Card className="border-zinc-800 bg-zinc-900/80"><CardHeader className="flex-row items-start justify-between"><div><CardTitle className="text-sm text-zinc-200">Heyserver-managed rules</CardTitle><p className="mt-1 text-[10px] text-zinc-500">Only rules in this section can be deleted from the panel</p></div><Button variant="ghost" size="xs" disabled={!online || firewallQuery.isFetching} onClick={() => firewallQuery.refetch()}><RefreshCw className={cn('size-3', firewallQuery.isFetching && 'animate-spin')} /> Refresh</Button></CardHeader><CardContent className="p-0">
      {managed.length === 0 ? <div className="p-10 text-center"><ShieldCheck className="mx-auto size-8 text-zinc-700" /><p className="mt-3 text-sm text-zinc-400">No Heyserver-managed firewall rules.</p></div> : <div className="divide-y divide-zinc-800/70">{managed.map((rule) => <RuleRow key={rule.id} rule={rule} onDelete={() => { if (window.confirm(`Delete firewall rule ${rule.id}?`) && firewallQuery.data?.revision) deleteMutation.mutate({ rule, revision: firewallQuery.data.revision }) }} pending={deleteMutation.isPending} online={online} writeAvailable={writeAvailable} controlsReady={controlsReady} />)}</div>}
    </CardContent></Card>
