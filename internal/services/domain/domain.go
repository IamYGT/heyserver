...
// integration is validated only when a request actually asks HServer to start
...
func (s *Service) create(req CreateRequest, result *CreateResult, hooks CreateHooks) error {
	...
	if req.IssueSSL {
		...
		if err := promoteDomainTLS(configPath, nginxContent, tlsContent, safeDomain); err != nil {
			...
			if result != nil {
				result.Warning = "Certificate issued, but HTTPS activation failed; HServer attempted HTTP rollback: " + err.Error()
			}
		} else if result != nil {
			result.HTTPSActive = true
		}
...
; PHP Version: %s
; Generated by HServer Panel — per-site isolation
...
; Resource limits (per portal — tune via HServer pool presets)
