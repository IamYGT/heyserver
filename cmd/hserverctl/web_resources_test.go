...
func TestRunWebResourceCommandsRejectUnsafeInputsBeforeRequest(t *testing.T) {
	...
	cases := []struct {
		...
		{args: []string{"nginx", "restore", "--confirm", "site.conf.hserver-archive-invalid"}, want: "exact Heyserver UTC timestamp"},
		...
		{args: []string{"nginx", "rollback", "--confirm", "site.conf.hserver-backup-invalid"}, want: "exact Heyserver UTC timestamp"},
