...
if [ "$#" -ne 3 ]; then
  printf '%s\n' "Usage: $0 ASSET PRIVATE_KEY OUTPUT" >&2
  exit 2
fi
...
[ -s "$asset" ] || { printf '%s\n' "Release asset not found: $asset" >&2; exit 1; }
...
mkdir -p "$parent"
umask 077
temporary=$(mktemp -d "$parent/.hserver-asset-signature.XXXXXX")
cleanup() {
  rm -rf "$temporary"
}
trap cleanup EXIT INT TERM

openssl pkeyutl -sign -rawin -inkey "$private_key" -in "$asset" \
  -out "$temporary/signature.raw" >/dev/null 2>&1
[ "$(wc -c <"$temporary/signature.raw" | tr -d ' ')" -eq 64 ] \
...
openssl pkeyutl -verify -pubin -inkey "$temporary/public.pem" -rawin \
  -in "$asset" -sigfile "$temporary/signature.raw" >/dev/null 2>&1 \
...
chmod 0644 "$output"
printf 'Release asset signature created: %s\n' "$output"
